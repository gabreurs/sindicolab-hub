  ...
  head: () => ({
    meta: [
      { title: "Solicitar acesso à Academy" },
      {
        name: "description",
        content:
          "Envie sua solicitação de acesso à Academy da sua administradora e aguarde a aprovação do responsável.",
      },
      { property: "og:title", content: "Solicitar acesso à Academy" },
      {
        property: "og:description",
        content: "Envie sua solicitação de acesso à Academy e aguarde a liberação.",
      },
    ...
    <div className="academy flex min-h-screen items-start justify-center p-6 sm:items-center">
      <div className="ax-panel w-full max-w-[560px] p-8">
        ...
        <p className="ax-body mt-1.5 text-[14px]">
          Envie seus dados para {org?.name ?? "a Academy"}. O responsável analisa a solicitação e
          libera seu acesso.
        </p>
        ...
        {state === "done" ? (
          <div className="mt-7">
            ...
            <p className="mt-2 text-sm ax-muted">
              Assim que for aprovada, você recebe um e-mail para criar sua senha e entrar na
              Academy.
            </p>
            <Link to="/" className="ax-btn mt-5 inline-flex" data-variant="secondary">
              Voltar ao início
            </Link>
          </div>
        ) : (
          <form onSubmit={submit} className="mt-7 grid gap-4">
            ...
            <label className="grid gap-1.5 text-sm">
              <span className="font-medium">
                Telefone <span className="ax-muted font-normal">(opcional)</span>
              </span>
            ...
            <label className="grid gap-1.5 text-sm">
              ...
              <input
                ...
                placeholder="Ex.: Síndico — Ed. Portal do Sol"
              />
            </label>
            <label className="grid gap-1.5 text-sm">
              <span className="font-medium">
                Informações complementares <span className="ax-muted font-normal">(opcional)</span>
              </span>
              <textarea rows={3} value={form.message} onChange={set("message")} />
            </label>
            {state === "error" && (
              <p className="text-sm" style={{ color: "var(--ax-danger, #b42318)" }}>
                {error}
              </p>
            )}
            <button
              type="submit"
              className="ax-btn justify-self-start"
              data-variant="primary"
              disabled={state === "busy" || !org?.id}
            >
